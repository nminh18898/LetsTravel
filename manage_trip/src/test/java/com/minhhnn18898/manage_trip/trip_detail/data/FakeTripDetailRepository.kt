...
class FakeTripDetailRepository: TripDetailRepository {
    ...
    private val flightInfosFlow: MutableSharedFlow<List<FlightInfoModel>> = MutableSharedFlow(replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)

    private val hotelInfoModelMap: LinkedHashMap<Long, HotelInfoModel> = LinkedHashMap()
    private val hotelInfoFlow: MutableSharedFlow<List<HotelInfoModel>> = MutableSharedFlow(replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)

    private val activityInfoModelMap: LinkedHashMap<Long, TripActivityInfoModel> = LinkedHashMap()
    private val activityInfoFlow: MutableSharedFlow<List<TripActivityInfoModel>> = MutableSharedFlow(replay = 1, onBufferOverflow = BufferOverflow.DROP_OLDEST)
    ...
    override fun getAllHotelInfo(tripId: Long): Flow<List<HotelInfo>> {
        if(forceError) {
            throw Exception("Internal Error. Can not get hotel info")
        }

        if(hotelInfoModelMap.isNotEmpty()) {
            return hotelInfoFlow.map {
                it.toHotelInfo()
            }
        }
    ...
    override suspend fun getHotelInfo(hotelId: Long): Flow<HotelInfo?> {
        ...
        if(hotelInfoModelMap.containsKey(hotelId)) {
            return hotelInfoFlow.map { hotelInfoModelMap[hotelId]?.toHotelInfo() }
        }
    ...
    override suspend fun insertHotelInfo(tripId: Long, hotelInfo: HotelInfo): Long {
        ...
        hotelInfoModelMap[currentId] = hotelInfo.toHotelInfoModel(tripId = tripId)

        hotelInfoFlow.tryEmit(hotelInfoModelMap.values.toList())
        autoGeneratedId++
    ...
    override suspend fun updateHotelInfo(tripId: Long, hotelInfo: HotelInfo) {
        if(forceError || !hotelInfoModelMap.containsKey(hotelInfo.hotelId)) {
            throw ExceptionUpdateHotelInfo()
        }

        hotelInfoModelMap[hotelInfo.hotelId] = hotelInfo.toHotelInfoModel(tripId = tripId)
        hotelInfoFlow.tryEmit(hotelInfoModelMap.values.toList())
    }

    override suspend fun deleteHotelInfo(hotelId: Long) {
        if(forceError || !hotelInfoModelMap.containsKey(hotelId)) {
            throw ExceptionDeleteHotelInfo()
        }

        hotelInfoModelMap.remove(hotelId)
    }
    ...
    override fun getSortedActivityInfo(tripId: Long): Flow<Map<Long?, List<TripActivityInfo>>> {
        ...
        if(activityInfoModelMap.isNotEmpty()) {
            return activityInfoFlow.map { listActivity ->
                listActivity
                    .toTripActivityInfo()
                    .sortedBy {
                        it.timeFrom
    ...
    override fun getActivityInfo(activityId: Long): Flow<TripActivityInfo?> {
        ...
        if(activityInfoModelMap.containsKey(activityId)) {
            return activityInfoFlow.map { activityInfoModelMap[activityId]?.toTripActivityInfo() }
        }
    ...
    override suspend fun insertActivityInfo(tripId: Long, activityInfo: TripActivityInfo): Long {
        ...
        activityInfoModelMap[currentId] = activityInfo.toTripActivityModel(tripId = tripId)

        activityInfoFlow.tryEmit(activityInfoModelMap.values.toList())
        autoGeneratedId++
    ...
    override suspend fun updateActivityInfo(tripId: Long, activityInfo: TripActivityInfo) {
        if(forceError || !activityInfoModelMap.containsKey(activityInfo.activityId)) {
            throw ExceptionUpdateTripActivityInfo()
        }

        activityInfoModelMap[activityInfo.activityId] = activityInfo.toTripActivityModel(tripId = tripId)
        activityInfoFlow.tryEmit(activityInfoModelMap.values.toList())
    }

    override suspend fun deleteActivityInfo(activityId: Long) {
        if(forceError || !activityInfoModelMap.containsKey(activityId)) {
            throw ExceptionDeleteTripActivityInfo()
        }

        activityInfoModelMap.remove(activityId)
    }

    fun reset() {
        flightInfoModelMap.clear()
        airportInfoMap.clear()
        hotelInfoModelMap.clear()
        activityInfoModelMap.clear()
        autoGeneratedId = 1L
        forceError = false
    ...
    fun getHotelInfoForTesting(hotelId: Long): HotelInfo? {
        return hotelInfoModelMap[hotelId]?.toHotelInfo()
    }

    @TestOnly
    fun addHotelInfo(tripId: Long, hotelInfo: HotelInfo) {
        hotelInfoModelMap[hotelInfo.hotelId] = hotelInfo.toHotelInfoModel(tripId)
        hotelInfoFlow.tryEmit(hotelInfoModelMap.values.toList())
    }

    @TestOnly
    fun updateHotelInfoForTesting(tripId: Long, hotelInfo: HotelInfo) {
        hotelInfoModelMap[hotelInfo.hotelId] = hotelInfo.toHotelInfoModel(tripId)
        hotelInfoFlow.tryEmit(hotelInfoModelMap.values.toList())
    }
    ...
    fun getActivityInfoForTesting(activityId: Long): TripActivityInfo? {
        return activityInfoModelMap[activityId]?.toTripActivityInfo()
    }

    @TestOnly
    fun addActivityInfo(tripId: Long, activityInfo: TripActivityInfo) {
        activityInfoModelMap[activityInfo.activityId] = activityInfo.toTripActivityModel(tripId)
        activityInfoFlow.tryEmit(activityInfoModelMap.values.toList())
    }

    @TestOnly
    fun updateActivityInfoForTesting(tripId: Long, activityInfo: TripActivityInfo) {
        activityInfoModelMap[activityInfo.activityId] = activityInfo.toTripActivityModel(tripId)
        activityInfoFlow.tryEmit(activityInfoModelMap.values.toList())
    }
}